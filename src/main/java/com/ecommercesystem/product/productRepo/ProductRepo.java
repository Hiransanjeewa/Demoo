...
public interface ProductRepo extends JpaRepository<product,Integer> {
    ...
    @Transactional
    @Modifying
    @Query(value="update wishlist a set a.item_id=?1 WHERE a.userid=?2", nativeQuery=true)
    void addWish(Integer item_id, Integer userid);

    @Transactional
    @Modifying
    @Query(value="delete wishlist a set a.item_id=?1 WHERE a.userid=?2", nativeQuery=true)
